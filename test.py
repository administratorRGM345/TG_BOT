...
# ================= НАЛАШТУВАННЯ =================
TOKEN = "8641628244:AAEaUSFQwAxYz26_qxdTFEiqmXP36bd1xtM"  # ⚠️ Перегенеруйте токен через @BotFather
APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxeUcz5V_zv-8LIf92Hg0_TEbFyj4HzkdGIKukbOLRrRMPTw86JmggtA3ZSDnzh_sxg/exec"  # напр. https://script.google.com/macros/s/XXXXX/exec
SECRET_KEY = "111"  # має збігатися з SECRET_KEY в Apps Script
ADMIN_PASSWORD = "admin123"  # 🔑 Пароль для адміністратора
...
async def admin_password_check(message: Message, state: FSMContext):
    if message.text and message.text.strip() == ADMIN_PASSWORD:
        ...
        if result.get("ok"):
            await message.answer(
                "✅ <b>Успішна авторизація!</b>\nСюди будуть надходити сповіщення про нові тікети.",
                parse_mode="HTML",
...
# ================= СПИСОК ТІКЕТІВ (АДМІН) =================
@router.message(F.text == BTN_TICKET_LIST)
async def show_ticket_list(message: Message, state: FSMContext):
    await message.answer("⏳ Завантажуємо список тікетів...")
    tickets = await get_open_tickets_remote()

    if not tickets:
        await message.answer("✅ Наразі немає невиконаних тікетів.", reply_markup=admin_menu_keyboard())
        return
    ...
    for t in tickets:
        ...
        date_time_line = f"{date_sheet_name} {t.get('time', '')}".strip()
        text = (
            f"🎫 <b>Тікет:</b> <code>{ticket_id}</code>\n"
            f"📅 <b>Дата/час:</b> {date_time_line}\n"
            ...
            f"📝 <b>Опис (прошу максимально детально описати проблему для якнайшвидшого вирішення проблеми):</b>\n{t.get('description')}\n\n{photo_text}"
        )
        await message.answer(
            text,
            parse_mode="HTML",
    ...
    await message.answer("👆 Це всі активні тікети.", reply_markup=admin_menu_keyboard())
...
async def mark_ticket_done(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await callback.answer("Помилка обробки тікета", show_alert=True)
        return
...
async def ticket_full_name(message: Message, state: FSMContext):
    ...
    await message.answer("<b>Крок 2/6:</b> Введіть Тему вашого звернення (проблему):", parse_mode="HTML")
...
async def finalize_ticket(message: Message, state: FSMContext):
    await message.answer("⏳ Зберігаємо тікет...")
